...
function convert() {
  ...
  if (valor > 0) {
    ...
    if (conv > unid) {
      calcM.innerHTML += " / ";
      calcM.innerHTML += (10 ** conv / 10 ** unid).toLocaleString("pt-BR");
    } else {
      calcM.innerHTML += " * ";
      calcM.innerHTML += (10 ** unid / 10 ** conv).toLocaleString("pt-BR");
    }

    if (massa > 10 ** 10) {
      respM.innerHTML =
        "<strong>Área:</strong> " + massa.toExponential() + " " + convText;
      calcM.innerHTML +=
        ") = " + massa.toExponential() + " " + convText + " </span>";
    } else if (massa > 10 ** 3) {
      respM.innerHTML =
        "<strong>Área:</strong> " +
        massa.toLocaleString("pt-BR") +
        " " +
        convText;
      calcM.innerHTML +=
